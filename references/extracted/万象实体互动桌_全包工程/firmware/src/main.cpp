...
void handleCommand(const std::string& command) {
    ...
    if (type == "MOVE") {
        ...
        // PIECE,FROM,ROW,3,COL,2,TO,ROW,5,COL,4
        ...
        // 注意：FROM和ROW是分开的参数，需要特殊处理
        // 实际格式: PIECE,FROM,3,2,TO,5,4
        if (args.size() >= 7) {
            fromRow = std::stol(args[2]);
            fromCol = std::stol(args[3]);
            toRow = std::stol(args[5]);
            toCol = std::stol(args[6]);
        }
